 ...
 */
import { generateIbanCheck, validateIban } from "./iban.js";
...
function verhoeffTally(value, offset = 0) {
    let c = 0;
...
function verhoeffCheck(payload) {
...
// ---------------------------------------------------------------------------
function weightedMod10Check(payload, weights) {
    let sum = 0;
...
function cusipCheck(payload) {
    let sum = 0;
    for (let i = 0; i < 8; i++) {
        const value = cusipCharValue(payload[i]);
        if (value === undefined)
            throw new Error(`invalid CUSIP character: ${payload[i]}`);
...
function numericScheme(meta, payloadLength, generator, validator, valuePattern = /^\d+$/, normalize) {
    const compute = validator ?? generator;
    return {
        meta,
        generate(payload) {
            if (!DIGITS.test(payload))
                throw new Error(`${meta.id} payload must be digits only`);
            if (payload.length !== payloadLength)
                throw new Error(`${meta.id} payload must be exactly ${payloadLength} digits, got ${payload.length}`);
...
export const SCHEMES = {
    luhn: {
        ...
        generate(payload) {
            if (!DIGITS.test(payload) || payload.length < 1)
                throw new Error('luhn payload must be at least 1 digit');
    ...
    verhoeff: {
        ...
        generate(payload) {
            if (!DIGITS.test(payload) || payload.length < 1)
                throw new Error('verhoeff payload must be at least 1 digit');
    ...
    damm: {
        ...
        generate(payload) {
            if (!DIGITS.test(payload) || payload.length < 1)
                throw new Error('damm payload must be at least 1 digit');
    ...
    isbn10: numericScheme({
        ...
        example: '0306406152',
    }, 9, isbn10Check, undefined, /^\d{9}[\dX]$/i, (v) => v.toUpperCase()),
    ...
    }, 12, (p) => weightedMod10Check(p, EAN13_WEIGHTS)),
    ean8: numericScheme({
        id: 'ean8',
        name: 'EAN-8',
        ...
        check: '8th digit',
        example: '96385074',
    }, 7, (p) => weightedMod10Check(p, EAN8_WEIGHTS)),
    ean13: numericScheme({
        id: 'ean13',
        name: 'EAN-13 (GTIN-13)',
    ...
    }, 11, (p) => weightedMod10Check(p, UPCA_WEIGHTS)),
    isin: {
        meta: {
            id: 'isin',
            name: 'ISIN (ISO 6166)',
        ...
        generate(payload) {
            ...
            if (!/^[A-Z]{2}[A-Z0-9]{9}$/.test(normalized))
                throw new Error('isin payload must be 2 letters + 9 alphanumerics');
    ...
    cusip: {
        ...
        generate(payload) {
            ...
            if (!/^[A-Z0-9*@#]{8}$/.test(normalized))
                throw new Error('cusip payload must be 8 chars from [A-Z0-9*@#]');
    ...
    cas: {
        ...
        generate(payload) {
            ...
            if (!DIGITS.test(p))
                throw new Error('cas payload must be digits only');
            if (p.length < 2 || p.length > 9)
                throw new Error(`cas payload must be 2-9 digits, got ${p.length}`);
            return casCheck(p);
        },
        validate(value) {
    ...
    aba: numericScheme({
        ...
        check: '9th digit',
        example: '021000021',
    }, 8, abaCheck, undefined, /^\d+$/, (v) => v.replace(/[\s-]/g, '')),
};
/**
 ...
 */
export function detectScheme(value) {
    ...
    if (/^\d{2,7}-\d{2}-\d$/.test(value.trim()))
        return 'cas';
    ...
    if (/^\d{9}$/.test(normalized) && abaPasses(normalized))
        return 'aba';
    if (CUSIP_RE.test(normalized))
        return 'cusip';
...
export function schemeMetaList() {
