 ...
 */
import { isbn10Check, weightedMod10Check, ISBN13_WEIGHTS } from "./core.js";
function digitGroups(raw) {
    return raw.split(/[\s-]+/).filter((g) => g !== '' && /^[\dX]+$/.test(g));
}
export function convertIsbn(input) {
    ...
    if (/^\d{9}[\dX]$/.test(normalized)) {
        ...
        const result = { valid: true, direction: 'to13', source: normalized, converted };
    ...
    if (/^\d{13}$/.test(normalized)) {
        ...
        const result = { valid: true, direction: 'to10', source: normalized, converted };
